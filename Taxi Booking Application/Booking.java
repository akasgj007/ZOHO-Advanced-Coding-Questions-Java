...
public class Booking {
    ...
    private static boolean isValidPickupAndDropPoints(char pickupPoint, char dropPoint) {
        return (pickupPoint > 'A' || pickupPoint < 'F' || dropPoint > 'A' || dropPoint < 'F');
    }

    private static List<Taxi> getFreeTaxis(List<Taxi> taxis, char pickupPoint, int pickupTime) {
        List<Taxi> freeTaxis = new ArrayList<>();
        for (Taxi t : taxis) {
            if (t.freeTime <= pickupTime // taxi should be free
                    &&
                    (Math.abs((t.currentSpot - '0') - (pickupPoint - '0')) <= (pickupTime - t.freeTime)) // taxi should
                                                                                                         // have enough
                                                                                                         // have time to
                                                                                                         // reach
                                                                                                         // customer
    ...
    private static void bookTaxi(int customerId, char pickupPoint, char dropPoint, int pickupTime,
            List<Taxi> freeTaxis) {
        // to find nearest
        int min = 999;
        ...
        for (Taxi t : freeTaxis) {
            int distanceBetweeenCustomerAndTaxi = Math.abs((t.currentSpot - '0') - (pickupPoint - '0')) * 15;
            ...
            while (true) {
                ...
                switch (option) {
                    case 1: {
                        ...
                        if (isValidPickupAndDropPoints(pickupPoint, dropPoint)) {
                            ...
                            System.out.println("Valid pickup and drop points are A,B,C,D,E,F.");
                            System.out.println("Exitting...");
                            return;
                        }
